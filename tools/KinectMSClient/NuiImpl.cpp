...
void CSkeletalViewerApp::Nui_GotSkeletonAlert( )
{
    ...
    for ( int i = 0 ; i < NUI_SKELETON_COUNT; i++ )
    {
		 ...
		 //####################   Socket entries
		boolean sendAbsPos=true; //send absolute position or position relative to root (hip)?

		 for( int j = 0 ; j < NUI_SKELETON_POSITION_COUNT ; j++ )
		{
			//send only tracked points
			if (SkeletonFrame.SkeletonData[i].eSkeletonPositionTrackingState[j] != NUI_SKELETON_POSITION_NOT_TRACKED){
				...
				z=SkeletonFrame.SkeletonData[i].SkeletonPositions[j].z;
				
				if (sendAbsPos){
					x=x+ SkeletonFrame.SkeletonData[i].Position.x;
					y=y+ SkeletonFrame.SkeletonData[i].Position.y;
					z=z + SkeletonFrame.SkeletonData[i].Position.z;
				}
				sprintf(str, "%d;%d;%g;%g;%g\n", j, i,x,y,z );
				server->sendMsg(str);
