...
//fuer FPS-Berechnung
    ...
    public void addPortMapEntry(int portNum, PinName name) throws ModulException
    {
        if (portMap.containsKey(portNum)) {
            throw new ModulException("Portmap-Eintrag bereits vorhanden: " + portNum);
        }
        if (portNum < 0 || portNum > 7) {
            System.err.println("WARNING: Portnum out of bounds:" + portNum);
        }
