...
import module.implementation.input.*;
import module.implementation.output.*;
import module.implementation.modify.*;
...
public Module createModulFromEntry(Element e) throws ModulException{
	...
	int id=parseID(e);
//	
	if (e.getAttribute("type")==null) throw new ModulFactoryException("Attribut 'type' fehlt bei Modul " + id);
	...
	if (e.getAttribute("Constructor")!= null) konstrParams=e.getAttribute("Constructor").getValue();
	Module m=getParameterizedInstance(type,konstrParams);
		
	if (e.getAttribute("graphXPos")!=null) {
		if (e.getAttribute("graphYPos")==null) throw new ModulFactoryException("Attribut 'graphYPos' fehlt, obwohl X-Koordinate angegeben wurde bei Modul " + id);
		int xpos,ypos;
	...
	if (maxModulID<id) maxModulID=id;
...
private static int parseID(Element e) throws ModulFactoryException {
	...
	if (e.getAttribute("id")==null) throw new ModulFactoryException("Attribut 'id' fehlt");
...
public Module createModulForGraph(String type) throws ModulException{
	...
	if (type.equals("MapPos Circle 2D") || type.equals("MapPos Square 2D")){
		...
		if (params==null) throw new ModulException("keine params");
		checkInt(points);
...
private static boolean inheritsFrom (Class c, Class superclass){
	...
	if (c.getSuperclass()==superclass) return true;
